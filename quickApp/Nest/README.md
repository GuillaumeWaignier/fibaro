...
6. (Optional) If you also want to use Nest Camera or Nest DoorBell, follow this [guide](doc/pubsub.md)
...
In the console log, you have this kind of log (you have to wait for 2 * frequency because each authentication step is performed in one loop).
...
After setting the code in the quickApp's parameter, if you get an error log like this, you may have a problem in **_"Client Secret"_** or **_"Authentication code"_**.
...
If you see an error related to PubSub Events, it concerns the motion detection of the Nest Camera.
A specific troubleshooting section is available [here](doc/pubsub.md#troubleshooting).


```bash
[06.01.2025] [10:37:25] [ERROR] [QUICKAPP245]: getPubSubEvent() ....
```
